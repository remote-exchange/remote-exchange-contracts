...
import {RemoteMinter__factory, RemoteVoter__factory, IERC20__factory} from "../../../typechain";
...
async function check(signer: SignerWithAddress, minter: string, token: string, veDist: string, voter: string) {
  const minterCtr = RemoteMinter__factory.connect(minter, signer);
  const tokenCtr = IERC20__factory.connect(token, signer);
  const voterCtr = RemoteVoter__factory.connect(voter, signer);

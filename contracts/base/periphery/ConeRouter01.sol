...
contract RemoteRouter01 {
  ...
  modifier ensure(uint deadline) {
    require(deadline >= block.timestamp, 'RemoteRouter: EXPIRED');
    _;
  }
  ...
  receive() external payable {
    ...
    require(msg.sender == address(wmatic), "RemoteRouter: NOT_WMATIC");
  }
  ...
  function _sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
    require(tokenA != tokenB, 'RemoteRouter: IDENTICAL_ADDRESSES');
    (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'RemoteRouter: ZERO_ADDRESS');
  }
  ...
  function _quoteLiquidity(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
    require(amountA > 0, 'RemoteRouter: INSUFFICIENT_AMOUNT');
    require(reserveA > 0 && reserveB > 0, 'RemoteRouter: INSUFFICIENT_LIQUIDITY');
  ...
  function _getAmountsOut(uint amountIn, Route[] memory routes) internal view returns (uint[] memory amounts) {
    require(routes.length >= 1, 'RemoteRouter: INVALID_PATH');
  ...
  function _addLiquidity(
    ...
    require(amountADesired >= amountAMin, "RemoteRouter: DESIRED_A_AMOUNT");
    require(amountBDesired >= amountBMin, "RemoteRouter: DESIRED_B_AMOUNT");
    // create the pair if it doesn't exist yet
    address _pair = IFactory(factory).getPair(tokenA, tokenB, stable);
    if (_pair == address(0)) {
    ...
    if (reserveA == 0 && reserveB == 0) {
      ...
      uint amountBOptimal = _quoteLiquidity(amountADesired, reserveA, reserveB);
      if (amountBOptimal <= amountBDesired) {
        require(amountBOptimal >= amountBMin, 'RemoteRouter: INSUFFICIENT_B_AMOUNT');
        (amountA, amountB) = (amountADesired, amountBOptimal);
      } else {
        uint amountAOptimal = _quoteLiquidity(amountBDesired, reserveB, reserveA);
        assert(amountAOptimal <= amountADesired);
        require(amountAOptimal >= amountAMin, 'RemoteRouter: INSUFFICIENT_A_AMOUNT');
        (amountA, amountB) = (amountAOptimal, amountBDesired);
      }
    }
  }
  ...
  function _removeLiquidity(
    ...
    require(amountA >= amountAMin, 'RemoteRouter: INSUFFICIENT_A_AMOUNT');
    require(amountB >= amountBMin, 'RemoteRouter: INSUFFICIENT_B_AMOUNT');
  }
  ...
  function swapExactTokensForTokensSimple(
    ...
    require(amounts[amounts.length - 1] >= amountOutMin, 'RemoteRouter: INSUFFICIENT_OUTPUT_AMOUNT');
  ...
  {
    require(routes[0].from == address(wmatic), 'RemoteRouter: INVALID_PATH');
  ...
  returns (uint[] memory amounts)
  {
    require(routes[routes.length - 1].to == address(wmatic), 'RemoteRouter: INVALID_PATH');
    amounts = _getAmountsOut(amountIn, routes);
    require(amounts[amounts.length - 1] >= amountOutMin, 'RemoteRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    IERC20(routes[0].from).safeTransferFrom(
      msg.sender, _pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    require(
      ...
      'RemoteRouter: INSUFFICIENT_OUTPUT_AMOUNT'
    );
  }
  ...
  ensure(deadline)
  {
    ...
    require(amountOut >= amountOutMin, 'RemoteRouter: INSUFFICIENT_OUTPUT_AMOUNT');
  ...
  function _safeTransferMATIC(address to, uint value) internal {
    ...
    require(success, 'RemoteRouter: ETH_TRANSFER_FAILED');
  }
}

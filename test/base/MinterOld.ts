...
import {Controller, Remote, RemoteMinter, Token, Ve, VeDist } from "../../typechain";
...
describe("minter old tests", function () {
  ...
  it("deploy base", async function () {
    ...
    ve_underlying = await Remote.deploy();
    ...
    const factory = await RemoteFactory.deploy();
    await factory.deployed();
    const RemoteRouter01 = await ethers.getContractFactory("RemoteRouter01");
    const router = await RemoteRouter01.deploy(factory.address, owner.address);
    ...
    const voter = await RemoteVoter.deploy(ve.address, factory.address, gauges_factory.address, bribe_factory.address);

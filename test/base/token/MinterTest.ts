...
import {RemotePair, Controller, Token} from "../../../typechain";
...
describe("minter tests", function () {
  ...
  it("wrong total amount test", async function () {
    ...
    const baseFactory = await Deploy.deployRemoteFactory(owner);
    ...
    const voter = await Deploy.deployRemoteVoter(owner, ve.address, baseFactory.address, gaugesFactory.address, bribesFactory.address);
    await controller.setVeDist(veDist.address)
    await controller.setVoter(voter.address)
    const minter = await Deploy.deployRemoteMinter(owner, ve.address, controller.address);
